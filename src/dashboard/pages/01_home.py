...
col1.metric(
    "Average ROE", f"{kpis['avg_roe']:.2f}%" if kpis["avg_roe"] is not None else "-"
)
col2.metric(
    "Median P/E", f"{kpis['median_pe']:.2f}" if kpis["median_pe"] is not None else "-"
)
col3.metric(
    "Median D/E", f"{kpis['median_de']:.2f}" if kpis["median_de"] is not None else "-"
)
